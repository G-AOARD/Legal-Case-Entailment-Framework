...
class Checkpoint(ColBERT):
    ...
    def queryFromText(self, queries, bsize=None, padding="longest", to_cpu=False, context=None,
                      full_length_search=False):
        if bsize:
            batches = self.query_tokenizer.tensorize(
                queries, context=context, bsize=bsize, padding=padding, full_length_search=full_length_search)
    ...
    def docFromText(self, docs, bsize=None, keep_dims=True, to_cpu=False, showprogress=False, return_tokens=False,
        ...
        if bsize:
            text_batches, reverse_indices = self.doc_tokenizer.tensorize(docs, bsize=bsize, padding=padding)
            ...
            input_ids_batches = [input_ids for input_ids, attention_mask in text_batches]
            if keep_dims is True:
                D = _stack_3D_tensors(batches)
                input_ids = _stack_2D_tensors(input_ids_batches)
        ...
        input_ids, attention_mask = self.doc_tokenizer.tensorize(docs, padding=padding)
