...
class DocTokenizer():
    ...
    def tensorize(self, batch_text, bsize=None, padding="longest"):
        assert type(batch_text) in [list, tuple], (type(batch_text))
        ...
        if isinstance(padding, int):
            tmp = self.tok(batch_text)["input_ids"]
            batch_maxlen = min(self.doc_maxlen, max([len(x) for x in tmp]) + padding)
            padding = "longest"
        else:
            batch_maxlen = self.doc_maxlen

        obj = self.tok(batch_text, padding=padding, truncation='longest_first',
                       return_tensors='pt', max_length=batch_maxlen).to(DEVICE)
